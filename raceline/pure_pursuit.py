...
class PurePursuit(Node):
    ...
    def __init__(self):
        ...
        self.pid = PIDController(kp = 1.0, ki = 0.0, kd = 0.0, history_length=10)

        #dimensions of the vehicle
        self.vehicle_width_meters           = 0.28
        self.vehicle_wheelbase_m            = 0.32
        ...
        self.raceline.load_trajectory_from_file("/home/itse/wette_racecar_ws/wohnzimmer_raceline.csv")
    ...
    def drive(self):
        ...
        # (x,y) is the target point, with is at angle alpha and distance lookahead measured from center front axle
        # use pure persuit formula to find the proper arc for computing the steering angle

        steering_angle = math.atan2(2.0 * self.vehicle_wheelbase_m * math.sin(alpha), lookahead)

        
        #use PID filter for steering signal
        alpha = self.pid.update(steering_angle)
